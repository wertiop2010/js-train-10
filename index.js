...
function customObjectFromEntries(entries) {
  ...
  const processedEntries = entries.map((entry) => {
    ...
    const processedKey = typeof key === "number" ? String(key) : key;
  ...
  // Перетворення числового значення на рядокл-
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const frozenObject = Object.freeze(extendedObject);
  if (Object.isFrozen(frozenObject)) {
    return frozenObject;
  } else {
    return "Помилка: Об'єкт не може бути заморожено.";
  }
...
function keysAndValues(obj) {
  ...
  return [keys, values];
  // Повертаємо масив з маси// Перевіряємо, чи вхідний параметр є об'єктом, якщо ні, повертаємо null
...
function checkAndPreventExtensions(obj) {
  ...
  var isExtensible = Object.isExtensible(obj);
  if (isExtensible) {
